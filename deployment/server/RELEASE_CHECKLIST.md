...
1. 用 `sanitize_rag_results.py` 生成公开 JSONL，并确认不存在会话 ID 或密钥字段。
2. 运行全部 Node、Python、30 条真实固定集与 6 条部署冒烟测试。
3. Compose 配置必须包含 Worker；后端 `7091` 只允许 Compose 内网访问。
4. 前端只绑定 `127.0.0.1:5173`，使用 HTTPS 反向代理、访问口令与 API 限流。
5. 正式回答只展示实际支撑答案的 Chunk；拒答必须为零 Sources。
6. `/demo`、`/ops/`、口语问法、边界问法和浏览器控制台全部通过后再更新公网链接。
