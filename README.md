...
- 业务问答页：[打开售后智能助手](https://124.221.243.125/demo/)
- 工程诊断台：[打开 RAG 评测与诊断台](https://124.221.243.125/ops/)
- 登录方式：浏览器 HTTP Basic Auth；演示用户名为 `Yancz`，访问口令由本人随简历或面试邀请单独提供，不写入公开仓库。
- 登录范围：同一服务器上的 DocsGPT RAG 与 DocFlow 共用一层演示访问保护。浏览器完成一次登录后可能自动复用凭据，因此切换项目时不一定再次弹出登录框。

业务问答页只展示问题、回答与实际采用的来源；工程诊断台用于查看版本对比、固定集指标和单条失败分析。`/preview/shared/` 仍保留为本地静态产品预览，用于验证界面、意图归一化和拒答文案，不作为公网主入口。
...
功能版本 `324769e` 已部署，Python 42/42、Node 61/61 自动化测试及 CI 通过，固定上游连续两次应用补丁与构建验证通过。修复上游 SSE 将来源正文截为前 100 字的问题后，云端 6 条指定真实 API 问答全部通过，来源数量依次为 1/1/1/0/0/0；运费来源正文保留金额及适用条件，3 类拒答零来源。

Chrome 页面加载、诊断台及双向导航正常，无 error/warn；本轮未额外执行浏览器问答。六问属于发布专项冒烟，不是重新执行全部 30 条或生产准确率。详见 [最终发布验收摘要](evaluation/reports/release-acceptance-2026-09-09.md)。
