...
> 复现并部署 DocsGPT，构建企业知识库 RAG 智能客服 Demo，完成知识库 Source 构建、Agent 配置、问答链路测试与问题排查。项目支持基于售后政策、会员规则和 FAQ 的可追溯问答，并定位修复“检索召回正确但生成未采纳上下文”的 Prompt 配置问题。

## 后续优化方向

- 接入更多真实业务文档，扩展多知识库场景。
- 构建批量测试集，统计召回准确率、答案正确率和拒答边界。
- 对比 DocsGPT 与 Quivr 在知识库管理、Agent 配置和问答体验上的差异。
- 增加自定义前端或企业客服工作台界面，提高 Demo 的产品完整度。
